...
function portableTextToHtml(blocks, imageUrlMap) {
  ...
  return blocks.map((block) => {
    try {
      ...
      if (block.style === "blockquote") {
        const text2 = block.children.map((child) => formatSpan(child)).join("");
        return `<blockquote class="border-r-4 border-[var(--accent-purple)] pr-4 my-4 text-[var(--text-secondary)] italic">${text2}</blockquote>`;
      }
      const text = block.children.map((child) => formatSpan(child)).join("");
      const styleMap = {
        h1: "h1",
        h2: "h2",
        h3: "h3",
